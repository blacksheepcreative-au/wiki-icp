<?php
/**
 * Template Name: Installation Videos
 */

get_header();

$post_type   = 'installation_video';
...
<main id="primary" class="site-main">
    <section class="tutorial-app" data-tutorial-app data-post-type="<?php echo esc_attr($post_type); ?>" data-default-id="<?php echo esc_attr($default_post_id); ?>">
        <button type="button" class="tutorial-mobile-toggle" data-tutorial-sidebar-toggle>
            <span><?php esc_html_e('Browse Installation Videos', 'wiki-icp'); ?></span>
        ...
        <div class="tutorial-shell">
            <aside class="tutorial-sidebar" data-tutorial-sidebar>
                <div class="tutorial-sidebar__header">
                    <div>
                        <p class="text-xs uppercase tracking-wide text-slate-400 mb-1"><?php esc_html_e('Installation Videos', 'wiki-icp'); ?></p>
                        <h2 class="text-lg font-semibold text-white m-0">
                            <?php echo esc_html($portal_term ? sprintf(__('%s Installation Library', 'wiki-icp'), $portal_term->name) : __('Installation Library', 'wiki-icp')); ?>
                        </h2>
                    </div>
                    <button type="button" class="tutorial-sidebar__close" data-tutorial-sidebar-close aria-label="<?php esc_attr_e('Close installation videos', 'wiki-icp'); ?>">
                ...
                <div class="tutorial-sidebar__body">
                    ...
                    if ($sidebar_markup) {
                        ...
                        echo '<p class="text-sm text-white/80">' . esc_html__('No installation videos available yet.', 'wiki-icp') . '</p>';
                    }
                    ?>
                </div>
            </aside>
                ...
                <div class="tutorial-content__inner">
                    ...
                    <div class="tutorial-body" data-tutorial-content>
                        ...
                        if ($default_content) {
                            ...
                            echo '<p>' . esc_html__('Select an installation video from the sidebar to begin.', 'wiki-icp') . '</p>';
                        }
                        ?>
                    </div>
                </div>
